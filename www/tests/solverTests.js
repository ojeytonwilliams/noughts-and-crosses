...
test("Horizontal win", t => {
  t.true(solver.hasWinner(game([[1,1], null, [1,2], null, [1,3]])), "A horizontal line should have won.");
});

test("Horizontal win, different order", t => {
  t.true(solver.hasWinner(game([[1,3], null, [1,1], null, [1,2]])), "The order of placement should not matter.");
})

test("A line must be complete (horizontal)", t => {
  t.false(solver.hasWinner(game([[1,3], null, [2,2], null, [1,1]])), "A line with a missing element is not a line.");
});

test("A line must be complete (vertical)", t => {
  t.false(solver.hasWinner(game([[3,1], null, [2,2], null, [1,1]])), "A line with a missing element is not a line.");
});
